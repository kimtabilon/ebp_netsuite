...
const netsuite_item_1 = require("../controller/netsuite_item");
const netsuite_po_1 = require("../controller/netsuite_po");
const netsuite_item_full_1 = require("../controller/netsuite_item_full");
const class_mapping_1 = require("../controller/class_mapping");
const item_field_mapping_1 = require("../controller/item_field_mapping");
const mongdodb_config_1 = require("../config/mongdodb.config");
const logger_config_1 = __importDefault(require("../config/logger.config"));
const router = (0, express_1.Router)();
// ─── Fetch items from NetSuite → netsuite.netsuite_product_list ─────────────
router.get("/netsuite-items", netsuite_item_1.syncNetsuiteItems);
// ─── Fetch POs from NetSuite → netsuite.netsuite_purchase_order ─────────────
router.get("/netsuite-po", netsuite_po_1.syncNetsuitePOs);
...
router.get("/netsuite-items-full", netsuite_item_full_1.syncNetsuiteItemsFull);
...
router.get("/map-item-classes", class_mapping_1.mapItemClasses);
...
router.get("/map-item-fields", item_field_mapping_1.mapItemFields);
// ─── Map isdropshipitem = false for all items ────────────────────────────────
// GET /map-item-dropship-false           → apply
// GET /map-item-dropship-false?dry=1     → dry-run
router.get("/map-item-dropship-false", item_field_mapping_1.mapDropshipFalse);
...
router.get("/push-item-fields", item_field_mapping_1.pushItemFields);
...
router.post("/reset-item-field-sync", async (_req, res) => {
    try {
        const nsDb = await (0, mongdodb_config_1.getDb)("netsuite");
        const col = nsDb.collection("netsuite_items_field_update");
        const result = await col.updateMany({}, { $unset: { ns_field_pushed: "", ns_field_pushed_at: "" } });
        logger_config_1.default.info(`[RESET-ITEM-FIELD-SYNC] Reset ${result.modifiedCount} documents`);
        res.json({ success: true, action: "reset", matched: result.matchedCount, modified: result.modifiedCount });
    }
    catch (e) {
        logger_config_1.default.error("[RESET-ITEM-FIELD-SYNC] Error:", e);
        res.status(500).json({ success: false, error: e.message });
    }
});
...
router.get("/erp-item-stats", async (req, res) => {
    try {
        ...
        const result = { success: true, total };
        if (beforeDate) {
        ...
        const samples = await col.find({ created: { $exists: true, $ne: null } }, { projection: { created: 1, lastmodifieddate: 1, itemid: 1, type: 1 } }).limit(3).toArray();
