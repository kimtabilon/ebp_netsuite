...
exports.mapItemClasses = mapItemClasses;
exports.runClassMapping = runClassMapping;
 ...
 */
async function mapItemClasses(req, res) {
    try {
        ...
        logger_config_1.default.error("[CLASS-MAP] Error:", err);
        return res.status(500).json({ success: false, error: err?.message ?? String(err) });
    }
}
async function runClassMapping(dryRun = false) {
    ...
    const classMap = new Map(); // fullname → internalid
    const nameAtLevelMap = new Map(); // "L1" or "L1 : L2" → internalid
    ...
    logger_config_1.default.info(`[CLASS-MAP] Classification map built: ${classMap.size} entries`);
    // ── Step 2: Build SKU → class levels map from master_list.product_list ──
    const productCol = masterDb.collection("product_list");
    const products = await productCol.find({}, { projection: { sku: 1, category_class: 1, category_class_l2: 1, category_class_l3: 1 } }).toArray();
    const skuClassMap = new Map();
    ...
    logger_config_1.default.info(`[CLASS-MAP] Master list loaded: ${skuClassMap.size} SKUs with class data`);
    // ── Step 3: Process items in netsuite_items_full ──
    const itemCol = nsDb.collection("netsuite_items_full");
    const items = await itemCol.find({ itemid: { $exists: true, $ne: null } }, { projection: { itemid: 1, class: 1, internalid: 1, type: 1 } }).toArray();
    logger_config_1.default.info(`[CLASS-MAP] Processing ${items.length} items from netsuite_items_full...`);
    let matched = 0;
    let updated = 0;
    ...
    if (!dryRun && ops.length > 0) {
        ...
        logger_config_1.default.info(`[CLASS-MAP] Wrote ${totalModified + totalUpserted} items to netsuite_items_field_update`);
    }
    const summary = {
        success: true,
        dryRun,
    ...
    logger_config_1.default.info(`[CLASS-MAP] Done — matched: ${matched}, updated: ${updated}, complete: ${alreadyComplete}, no class: ${noClassInMaster}`);
    return summary;
}
