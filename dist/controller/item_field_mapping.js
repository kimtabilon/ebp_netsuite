...
exports.mapItemFields = mapItemFields;
exports.mapDropshipFalse = mapDropshipFalse;
exports.pushItemFields = pushItemFields;
 ...
 */
async function mapItemFields(req, res) {
    try {
        ...
        logger_config_1.default.error("[FIELD-MAP] Error:", err);
...
async function runFieldMapping(dryRun = false) {
    ...
    const products = await productCol.find({}, { projection: { sku: 1, name: 1, product_name: 1, displayname: 1 } }).toArray();
    ...
    logger_config_1.default.info(`[FIELD-MAP] Master list loaded: ${skuMasterMap.size} SKUs with names`);
    // ── Step 2: Load netsuite_items_full ──
    const itemCol = nsDb.collection("netsuite_items_full");
    const items = await itemCol.find({ itemid: { $exists: true, $ne: null } }, { projection: { itemid: 1, internalid: 1, displayname: 1, purchasedescription: 1, type: 1 } }).toArray();
    logger_config_1.default.info(`[FIELD-MAP] Processing ${items.length} items from netsuite_items_full...`);
    ...
    if (!dryRun && ops.length > 0) {
        ...
        for (let i = 0; i < ops.length; i += BATCH_SIZE) {
            ...
            const r = await updateCol.bulkWrite(batch, { ordered: false });
        ...
        logger_config_1.default.info(`[FIELD-MAP] Wrote ${totalModified + totalUpserted} items to netsuite_items_field_update`);
    }
    const summary = {
        success: true,
        dryRun,
    ...
    logger_config_1.default.info(`[FIELD-MAP] Done — matched: ${matched}, needsUpdate: ${needsUpdate}, alreadyCurrent: ${alreadyCurrent}, noMatch: ${noMatch}`);
    return summary;
}
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
async function mapDropshipFalse(req, res) {
    try {
        const dryRun = req.query.dry === "1" || req.query.dry === "true";
        ...
        logger_config_1.default.error("[DROPSHIP-MAP] Error:", err);
        return res.status(500).json({ success: false, error: err?.message ?? String(err) });
    }
}
async function runDropshipMapping(dryRun = false) {
    ...
    const dropshipItems = await itemCol.find({ isdropshipitem: { $in: ["T", true, "Yes", "t"] } }, { projection: { internalid: 1, itemid: 1, type: 1, isdropshipitem: 1 } }).toArray();
    logger_config_1.default.info(`[DROPSHIP-MAP] Found ${dropshipItems.length} items with isdropshipitem = true`);
    if (dryRun) {
        return {
            success: true,
            dryRun: true,
    ...
    logger_config_1.default.info(`[DROPSHIP-MAP] Wrote ${totalWritten} items to netsuite_items_field_update`);
    return {
        success: true,
        dryRun: false,
        count: dropshipItems.length,
 ...
 */
async function pushItemFields(req, res) {
    try {
        const dryRun = req.query.dry === "1" || req.query.dry === "true";
        ...
        logger_config_1.default.error("[FIELD-PUSH] Error:", err);
        return res.status(500).json({ success: false, error: err?.message ?? String(err) });
    }
}
async function runFieldPush(dryRun = false) {
    ...
    const FIELD_KEYS = ["displayname", "purchasedescription", "isdropshipitem", "class"];
    const mrItems = [];
    ...
    logger_config_1.default.info(`[FIELD-PUSH] ${mrItems.length} items to push${dryRun ? " [DRY RUN]" : ""}`);
    ...
    if (!mrResult?.success) {
        logger_config_1.default.error(`[FIELD-PUSH] Failed to trigger MapReduce:`, mrResult?.error);
        return {
            success: false,
    ...
    logger_config_1.default.info(`[FIELD-PUSH] MapReduce triggered — taskId: ${mrResult.taskId}, fileId: ${mrResult.fileId}, items: ${mrResult.itemCount}, fields: ${mrResult.fields}`);
    // Poll for completion
    let status = "PENDING";
    ...
    for (let poll = 0; poll < MAX_POLLS; poll++) {
        ...
        try {
            const statusResponse = await (0, netsuite_client_1.callDiagnostic)({
            ...
            logger_config_1.default.info(`[FIELD-PUSH] MR status: ${status} (${percentComplete}%)`);
            if (status === "COMPLETE" || status === "FAILED")
                break;
        }
        catch (pollErr) {
            logger_config_1.default.warn(`[FIELD-PUSH] Poll error: ${pollErr.message}`);
        }
    }
    // Mark all items as pushed in MongoDB
    if (status === "COMPLETE") {
        const allIds = mrItems.map((r) => r.id);
        await updateCol.updateMany({ internalid: { $in: allIds } }, { $set: { ns_field_pushed: true, ns_field_pushed_at: new Date() } });
        logger_config_1.default.info(`[FIELD-PUSH] Marked ${allIds.length} items as ns_field_pushed in MongoDB`);
    }
    return {
