...
exports.syncAllWare2GoInboundShipments = syncAllWare2GoInboundShipments;
...
//     }
// }
async function syncAllWare2GoInboundShipments(options = {}) {
    try {
        const { forceRestart = false, retryFailedOnly = false, startDate = null, all = false } = options;
        const username = process.env.W2G_USERNAME || '';
        const password = process.env.W2G_PASSWORD || '';
        const merchantId = process.env.W2G_MERCHANT_ID || '';
        let FINAL_STATUSES = [];
        ...
        const db = await (0, mongdodb_config_1.getDb)("ebp_w2g");
        const inboundsCollection = db.collection("w2g_inbound");
        const syncMetaCollection = db.collection("w2g_sync_meta");
        await inboundsCollection.createIndex({ shipmentId: 1 }, { unique: true });
        const RATE_LIMIT_DELAY = 200;
            ...
            if (shipmentIdsToFetch.length === 0) {
                ...
                while (hasMorePages && page <= MAX_PAGES) {
                    ...
                    for (const shipment of shipments) {
                        const id = shipment.shipmentId || shipment.id || shipment.inboundShipmentId;
                        if (id)
                            shipmentIdsToFetch.push(id);
                    }
                    await syncMetaCollection.updateOne({ merchantId, type: 'inbound_sync' }, {
                        $set: {
                            lastProcessedPage: page,
                            totalShipmentIds: shipmentIdsToFetch,
        ...
        const existingShipments = await inboundsCollection.find({ shipmentId: { $in: shipmentIdsToFetch } }, { projection: { shipmentId: 1, status: 1 } }).toArray();
        const existingMap = new Map();
        for (const shipment of existingShipments) {
            existingMap.set(shipment.shipmentId, shipment.status);
        }
        const idsToFetchDetails = [];
        const idsToSkip = [];
        const idsNew = [];
        for (const shipmentId of shipmentIdsToFetch) {
            ...
            if (!existingStatus) {
                ...
                const isFinal = FINAL_STATUSES.some(final => existingStatus.toLowerCase() === final);
                if (isFinal) {
        ...
        await syncMetaCollection.updateOne({ merchantId, type: 'inbound_sync' }, { $set: { status: 'running', startedAt: new Date() } }, { upsert: true });
        async function processBatch(batch, batchIndex) {
            for (const shipmentId of batch) {
                ...
                try {
                    // Fetch full shipment details
                    const detailResponse = await (0, axios_1.default)({
                        method: 'get',
                        url: `https://openapi.ware2go.io/v1/merchants/${merchantId}/inbound_shipments/${shipmentId}`,
                        auth: { username, password },
                        timeout: 30000
                    ...
                    while (!dbSuccess && dbRetries > 0) {
                        try {
                            const dbResult = await inboundsCollection.updateOne({ shipmentId: shipmentId }, {
                                $set: { ...fullShipment, _updatedAt: new Date() },
                                $setOnInsert: { _createdAt: new Date(), _source: 'ware2go_api' }
                            }, { upsert: true });
                    ...
                    console.log(`✅ [W${batchIndex}] ${shipmentId} | ` +
        ...
        for (let i = 0; i < batches.length; i++) {
            const batch = batches[i];
            await Promise.all(batch.map((_, workerIndex) => processBatch([batch[workerIndex]], workerIndex)));
            if (results.detailsProcessed % DETAIL_BATCH_SIZE === 0) {
                await syncMetaCollection.updateOne({ merchantId, type: 'inbound_sync' }, {
                    $set: {
                        lastProcessedShipmentIndex: results.detailsProcessed,
        ...
        console.error("\n❌ [W2G INBOUND] Fatal error:", error.message);
        const db = await (0, mongdodb_config_1.getDb)("w2g_inbound");
        await db.collection("w2g_sync_meta").updateOne({ merchantId: process.env.W2G_MERCHANT_ID, type: 'inbound_sync' }, { $set: { status: 'error', error: error.message, errorAt: new Date() } }, { upsert: true });
        if (error.response) {
            console.error("Status:", error.response.status);
            console.error("Data:", JSON.stringify(error.response.data, null, 2));
        }
