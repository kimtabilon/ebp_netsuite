...
exports.getWare2SoOrderOutbound = getWare2SoOrderOutbound;
...
//     }
// }
async function getWare2SoOrderOutbound(options = {}) {
    try {
        ...
        if (retryFailedOnly) {
            ...
            if (orderIdsToFetch.length === 0) {
                ...
                while (hasMorePages && page <= MAX_PAGES) {
                    ...
                    await syncMetaCollection.updateOne({ merchantId, type: 'full_sync' }, {
                        $set: {
                            lastProcessedPage: page,
                            totalOrderIds: orderIdsToFetch,
                            updatedAt: new Date()
        ...
        const existingOrders = await ordersCollection.find({ orderId: { $in: orderIdsToFetch } }, { projection: { orderId: 1, status: 1 } }).toArray();
        ...
        // Categorize
        const idsToFetchDetails = []; // Need API call
        const idsToSkip = []; // Already final status
        const idsNew = []; // Don't exist
        ...
        await syncMetaCollection.updateOne({ merchantId, type: 'full_sync' }, { $set: { status: 'running', startedAt: new Date() } }, { upsert: true });
            ...
            for (const orderId of batch) {
                try {
                    ...
                    while (!dbSuccess && dbRetries > 0) {
                        try {
                            const dbResult = await ordersCollection.updateOne({ orderId: orderId }, {
                                $set: { ...fullOrder, _updatedAt: new Date() },
                                $setOnInsert: { _createdAt: new Date(), _source: 'ware2go_api' }
                            }, { upsert: true });
                            if (dbResult.upsertedCount) {
        ...
        console.error("\n❌ [W2G API] Fatal error:", error.message);
        const db = await (0, mongdodb_config_1.getDb)("ebp_w2g");
        await db.collection("w2g_sync_meta").updateOne({ merchantId: process.env.W2G_MERCHANT_ID, type: 'full_sync' }, { $set: { status: 'error', error: error.message, errorAt: new Date() } }, { upsert: true });
        if (error.response) {
            console.error("Status:", error.response.status);
            console.error("Data:", JSON.stringify(error.response.data, null, 2));
        }
