...
exports.syncCreditMemosToNetsuite = syncCreditMemosToNetsuite;
exports.deletesyncBills = deletesyncBills;
 ...
 */
async function syncCreditMemosToNetsuite() {
    logger_config_1.default.info("[NS Credit Sync] Starting via RESTlet...");
    ...
    const credits = await collection.find(filter).limit(1).toArray();
    logger_config_1.default.info(`[NS Credit Sync] Found ${credits.length} records to sync.`);
    for (const credit of credits) {
        try {
            logger_config_1.default.info(`[NS Credit Sync] Processing Ref: ${credit.reference_number} (PO ${credit.po_number})`);
            // Resolve Location ID
            ...
            const payload = {
                ...
                line_items: credit.items.map((item) => ({
                    sku: item.sku,
                    qty: item.qty,
                    price: item.price,
            ...
            const result = await (0, netsuite_client_1.postToNetsuiteForCreditMemo)(payload);
            if (result.success) {
                logger_config_1.default.info(`✅ [NS Credit Sync] Success: ${credit.reference_number} -> NS ID ${result.internalId}`);
                await collection.updateOne({ _id: credit._id }, {
                    $set: {
                        ns_synced: true,
            ...
            logger_config_1.default.error(`❌ [NS Credit Sync] Failed for ${credit.reference_number}: ${errMsg}`);
            await collection.updateOne({ _id: credit._id }, {
                $set: {
                    ns_failed: true,
                    ns_error: errMsg,
    ...
    logger_config_1.default.info("[NS Credit Sync] Finished.");
}
async function deletesyncBills() {
    try {
        ...
        if (matchRecords.length === 0) {
            logger_config_1.default.info("No record found");
        ...
        // });
        logger_config_1.default.info("Successful", {
            success: true,
            total_found: matchRecords.length,
            deleted_count: deletedIds.length,
            failed_count: errors.length,
