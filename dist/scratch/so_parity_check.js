...
async function checkParity() {
    ...
    const DUMP_COL = "so_dump_test";
    const SUITE_COL = "suite_sales_order";
    logger_config_1.default.info("\n=== Sales Order Parity Check: Dump vs Dummy ===\n");
    // 1. Fetch data
    const [dumpDocs, suiteDocs] = await Promise.all([
        db.collection(DUMP_COL).find({}).project({ "so.otherRefNum": 1 }).toArray(),
        db.collection(SUITE_COL).find({ ns_synced: true }).project({ otherrefnum: 1 }).toArray()
    ]);
    ...
    for (const d of dumpDocs) {
        const key = normalizeRef(d.so?.otherRefNum);
        if (key)
            dumpMap.set(key, d);
    }
    const suiteMap = new Map();
    for (const s of suiteDocs) {
        const key = normalizeRef(s.otherrefnum);
        if (key)
            suiteMap.set(key, s);
    }
    // 3. Find Differences
