...
async function run() {
    ...
    const sql = "SELECT date, title, detail FROM ScriptNote WHERE date >= '5/29/2026' ORDER BY date DESC FETCH FIRST 50 ROWS ONLY";
    try {
        const response = await axios_1.default.post(url, { q: sql }, {
            headers: {
        ...
        console.log("Recent Logs:", JSON.stringify(items, null, 2));
    }
    catch (error) {
