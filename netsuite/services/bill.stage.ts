...
import { poExistsInNetSuite } from "./netsuite.helpers";
...
export interface StagedBill {
    ...
    total_amount: string;
    items: { sku: string; qty: string; price: string; serials?: string[] }[];
    summary: any;
    terms: string;
    payment_type: string;
    po_type: string;
    ...
    // Change detection flag (internal, not stored)
    _changed?: boolean;
}

// ─── Shared helpers ────────────────────────────────────────────────────────────

function buildItemsArray(rawItems: any[]): { sku: string; qty: string; price: string; serials?: string[] }[] {
    const items: { sku: string; qty: string; price: string; serials?: string[] }[] = [];
...
const BILL_CONTENT_FIELDS: (keyof StagedBill)[] = [
    "invoice_number",
    "distributor",
    "vendor_id",
    "invoice_type",
...
function isStagedBillContentEqual(a: any, b: any): boolean {
...
function buildBulkOp(bill: StagedBill, changed: boolean = false): object {
    const { ns_skip, ns_skip_reason, _changed, ns_synced, ns_sync, ns_result, ns_error, ns_error_at, last_synced_at, ns_retry_count, staged_at, ...dataFields } = bill as any;
    const now = new Date();
...
export const stageBills = async (): Promise<{ processed: number; skipped: number; reasons: Record<string, number> }> => {
    log.info(`[Bill Stage Dummy] Starting streaming approach...`);
    const po_db = await getDb("ebp_pomanager");
    const ns_db = await getDb("netsuite");
    const collection = ns_db.collection<StagedBill>("suite_vendor_bill_dummy");
    ...
    log.info(`[Bill Stage Dummy] Running aggregation to detect duplicate invoice numbers...`);
    ...
    if (duplicatedInvoiceNumbers.size > 0) {
        log.warn(`[Bill Stage Dummy] Found ${duplicatedInvoiceNumbers.size} duplicated invoiceNumbers.`);
    }
    ...
    function markSkip(doc: Partial<StagedBill>, reason: string) {
        doc.ns_skip = true;
        doc.ns_skip_reason = reason;
        skippedCount++;
        skipReasons[reason] = (skipReasons[reason] || 0) + 1;
        log.warn(`[Bill Stage Dummy][SKIP] PO: ${doc.po_number || "-"} | Invoice: ${doc.invoice_number || "-"} | Reason: ${reason}`);
    }

    log.info(`[Bill Stage Dummy] Streaming bills from cursor in high-speed batches...`);

    let batch: any[] = [];

    async function processBatch(currentBatch: any[]) {
        const bulkOps: any[] = [];
        ...
        for (const bill of currentBatch) {
            ...
            const doc: StagedBill = {
                ...
                ...({ po_exist, po_sync } as any)
            };

            // Validation
            if (!dueDate) markSkip(doc, "no_dueDate");
            else if (items.length === 0) markSkip(doc, "no_items");
            else if (!invoiceDate) markSkip(doc, "no_invoiceDate");
    ...
    log.info(`[Bill Stage Dummy] Done — total=${total}, processed=${processed}, updated=${updated}, unchanged=${unchangedCount}, skipped=${skippedCount}`);
...
export const stageCreditBillsDummy = async (): Promise<{ processed: number; skipped: number; reasons: Record<string, number> }> => {
    log.info(`[Credit Bill Stage Dummy] Starting... (workers: ${STAGE_DUMMY_WORKERS})`);
    const po_db = await getDb("ebp_pomanager");
    const ns_db = await getDb("netsuite");
    const collection = ns_db.collection<StagedBill>("suite_credit_memo_bill");
    const poCollection = ns_db.collection("suite_purchase_order_dummy");
    ...
    log.info(`[Credit Bill Stage Dummy] Found ${billCount} bills matching criteria`);

    const invoiceNumberCount: Record<string, number> = {};
    for (const bill of bills) {
        const invNum = String(bill.invoiceNumber);
        invoiceNumberCount[invNum] = (invoiceNumberCount[invNum] || 0) + 1;
    }
    const duplicatedInvoiceNumbers = new Set(
        Object.entries(invoiceNumberCount)
    ...
    function markSkip(doc: StagedBill, reason: string) {
        ...
        skippedCount++;
        skipReasons[reason] = (skipReasons[reason] || 0) + 1;
        log.warn(`[Credit Bill Stage][SKIP] PO: ${doc.po_number} | Invoice: ${doc.invoice_number} | Reason: ${reason}`);
    }
    ...
    log.info(`[Credit Bill Stage Dummy] Performing bulk database lookups for ${candidates.length} bills...`);
    ...
    log.info(`[Credit Bill Stage Dummy] Processing staging data in memory...`);
    for (const bill of candidates) {
        ...
        if (poDoc && Array.isArray(poDoc.order_items)) {
            for (const bItem of rawItems) {
                const poItem = poDoc.order_items.find((p: any) => p.sku === bItem.sku);
                if (poItem) {
    ...
    if (bulkOps.length > 0) {
        try {
            ...
            for (let i = 0; i < bulkOps.length; i += CHUNK_SIZE) {
                ...
                await collection.bulkWrite(chunk as any, { ordered: false });
            }
            log.info(`[Credit Bill Stage Dummy] BulkWrite done — newly staged: ${changedCount}`);
        } catch (dbErr: any) {
            log.error(`[Credit Bill Stage Dummy] BulkWrite ERROR: ${dbErr.message}`);
            if (dbErr.writeErrors) {
                log.error(`[Credit Bill Stage Dummy] First write error: ${JSON.stringify(dbErr.writeErrors[0])}`);
            }
        }
    }

    log.info(`[Credit Bill Stage Dummy] Done. Staged ${staged.length} bills.`);
    return {
...
export async function runFunctionForBills() {
    try {
        const ns_db = await getDb("netsuite");
        ...
        const unsyncedDummySet = new Set<string>();
        const ghostBills: string[] = [];
        ...
        const existInDumpButUnsyncedInDummy: string[] = [];
        for (const refNum of dumpSet) {
