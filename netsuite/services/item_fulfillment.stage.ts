...
export const IF_COLLECTION = "suite_item_fulfillment";

export interface StagedItemFulfillment {
    po_number: number;
...
export const stageItemFulfillmentsDummy = async (): Promise<{
    processed: number;
    skipped: number;
    reasons: Record<string, number>;
}> => {
    log.info("[IF Stage Dummy] Starting...");

    const po_db = await getDb("ebp_pomanager");
    const ns_db = await getDb("netsuite");
    const collection = ns_db.collection<StagedItemFulfillment>(IF_COLLECTION);
    ...
    log.info(`[IF Stage Dummy] Found ${pos.length} Dropship Shipped POs`);

    if (pos.length === 0) return { processed: 0, skipped: 0, reasons: {} };

    // --- OPTIMIZATION: BATCH FETCH ALL DEPENDENCIES ---
    log.info(`[IF Stage Dummy] Batch fetching dependencies...`);
    ...
    log.info(`[IF Stage Dummy] Dependency maps ready.`);

    const staged: StagedItemFulfillment[] = [];
    let skippedCount = 0;
    const skipReasons: Record<string, number> = {};

    function markSkip(ref: string, reason: string) {
        skippedCount++;
    ...
    for (const po of pos) {
        ...
        if (!websiteOrderNum) { markSkip(String(poNum), "no_website_order_number"); continue; }
        ...
        if (!trackingNumber) { markSkip(String(poNum), "no_tracking_number"); continue; }
        ...
        if (!po_synced) { markSkip(String(poNum), "po_not_synced"); continue; }
        if (!so_synced) { markSkip(String(poNum), "so_not_synced"); continue; }

        // --- SERIAL BUCKET PREP ---
        let serialBucket: string[] = [];
        ...
        let serialPtr = 0;
        const items = (po.order_items || []).map((item: any) => {
        ...
        if (items.length === 0) { markSkip(String(poNum), "no_items"); continue; }
        ...
        const doc: StagedItemFulfillment = {
            po_number: poNum,
    ...
    log.info(`[IF Stage Dummy] Done. Staged: ${staged.length} | Skipped: ${skippedCount}`);
